...
class GraphQlApiService(
    ...
    override fun <TEntity> operation(request: GraphQlRequest<TEntity>): Publisher<NetworkResult<TEntity>> {
        ...
        return if (authenticationContext.isAvailable()) {
            ...
            Publishers.just(
                NetworkResult.Error(
                    Throwable("RoverCampaigns API authentication not available."),
                    true
                )
            )
        }
    ...
    override fun submitEvents(events: List<EventSnapshot>): Publisher<NetworkResult<String>> {
        return if (!authenticationContext.isAvailable()) {
            log.w("Events may not be submitted without a RoverCampaigns authentication context being configured.")
            Publishers.just(
                NetworkResult.Error(
                    Exception("Attempt to submit Events without RoverCampaigns authentication context being configured."),
                    false
                )
            )
        } else {
